...
Future<void> buildWindows(
  ...
  final Directory buildDirectory =
      globals.fs.directory(getWindowsBuildDirectory());
  ...
  try {
    ...
    if (visualStudio.displayVersion == '17.1.3') {
...
Future<void> buildWindowsUwp(
  WindowsUwpProject windowsProject,
  BuildInfo buildInfo, {
  String? target,
  VisualStudio? visualStudioOverride,
}) async {
  final Directory buildDirectory =
      globals.fs.directory(getWindowsBuildUwpDirectory());
...
Future<void> _runFlutterBuild(
    Directory buildDirectory, BuildInfo buildInfo, String? targetFile) async {
    ...
    result = await globals.processUtils.stream(
      <String>[
        ...
        if (buildInfo.extraGenSnapshotOptions != null &&
            buildInfo.extraGenSnapshotOptions.isNotEmpty)
